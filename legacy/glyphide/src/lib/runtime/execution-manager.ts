...
const debouncedExecute = debounce(() => {
	if (
		...
		if (canStartExecution()) {
			console.log(
				"ExecutionManager: Debounced execution triggered for code change.",
			);
			lastExecutedCode = lastCodeToExecute;
			executeCode(lastCodeToExecute);
		} else {
			console.log(
				"ExecutionManager: Debounced execution ignored, state not allowed to start.",
			);
			const currentStatus = getAppStatus();
		...
		console.log(
			"ExecutionManager: Debounced execution skipped, no code or no changes.",
		);
		const currentStatus = getAppStatus();
		if (currentStatus === "waiting") {
			setAppStatus("idle");
...
export const triggerExecutionImmediately = (code: string) => {
	...
	if (canStartExecution() && code && code.trim().length > 0) {
		...
		console.log(
			"ExecutionManager: Immediate execution ignored, state not allowed or no code.",
		);
	}
};
