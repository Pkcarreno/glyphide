...
class ExecutionController {
	...
	initialize(): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			const initValidationResult =
				MainToWorkerMessageSchema.safeParse(initMessage);
	...
	private handleMessageFromWorker = (event: MessageEvent<unknown>) => {
		...
		if (!validationResult.success) {
			...
			addLog({
				type: "error",
				outputs: [
					`Invalid message from worker: ${JSON.stringify(validationResult.error.issues)}`,
				],
			});
			return;
		}
		...
		switch (message.type) {
			...
			case MessageTypes.EXECUTION_RESULT: {
				console.log(
					"ExecutionController: Received execution result.",
					message.payload,
				);
				setAppStatus("idle");
			...
			case MessageTypes.EXECUTION_OUTPUT: {
				...
				console.log(
					"ExecutionController: Received execution output.",
					outputPayload,
				);
			...
			case MessageTypes.EXECUTION_ERROR: {
				...
				console.error(
					"ExecutionController: Received worker process error.",
					errorPayload,
				);
				addLog({
					type: "error",
			...
			default: {
				console.warn(
					"ExecutionController: Received unhandled valid message: ",
					message,
				);
				break;
			}
	...
	executeCode(code: string) {
		...
		const sendValidationResult =
			MainToWorkerMessageSchema.safeParse(executionMessage);
	...
	terminateExecution() {
		if (this.messageChannel) {
			const terminateMessage: MainToWorkerMessage = {
				type: MessageTypes.TERMINATE_EXECUTION,
			};
...
export const executeCode = (code: string) =>
	executionControllerInstance.executeCode(code);
export const terminateExecution = () =>
	executionControllerInstance.terminateExecution();
