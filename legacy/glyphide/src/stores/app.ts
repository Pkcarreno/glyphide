...
function getInstanceKey(): string {
	instanceCounter++;
	return (
		instanceCounter.toString(36).padStart(4, "0") +
	...
	addLog: (payload) => {
		set((state) => {
			...
			if (
				...
				lastLog._outputKey === currentLogKey
			) {
...
export const setAppStatus = (status: appStatusType) =>
	_useAppStore.getState().setStatus(status);
export const addLog = (log: ExecutionOutputPayload) =>
	_useAppStore.getState().addLog(log);
...
export const getPersistLogsStatus = () =>
	_useAppPersistStore.getState().persistLogs;
