...
const CodemirrorEditor: React.FC<CodeMirrorEditorProps> = ({
			...
			if (onEditorStateChange) {
				...
				const currentInfo: editorStateType = {
					...
					selectionLength: selection.ranges.reduce(
						(len: number, range: SelectionRange) =>
							len + (range.to - range.from),
						0,
					),
					cursorPosition:
						selection.main.head - doc.lineAt(selection.main.head).from,
				};

				if (
					currentInfo.cursorPosition !==
						lastEditorStateInfo.current.cursorPosition ||
					currentInfo.currentLine !== lastEditorStateInfo.current.currentLine ||
					currentInfo.selectionLength !==
						lastEditorStateInfo.current.selectionLength
				) {
	...
	useEffect(() => {
		if (
			...
			editorViewRef.current.state.doc.toString() !== initialDoc
		) {
			editorViewRef.current.dispatch(
